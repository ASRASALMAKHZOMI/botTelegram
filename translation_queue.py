...
# =========================
# 🔥 Rate Limiter مركزي (المسؤولية هنا فقط)
# =========================
last_request_time = 0

def wait_rate_limit(min_interval=30):  # 🔥 زدنا إلى 30 ثانية للحماية القصوى
...
# =========================
# 🔥 ترجمة Batch آمنة (هنا فقط الـ Retry Logic)
# =========================
def safe_translate_batch(pages_batch):
    ...
    for attempt in range(max_retries):
        try:
            wait_rate_limit()  # 🔥 الانتظار قبل كل طلب
            
            result = translate_batch(pages_batch)  # 🔥 طلب نظيف بدون Retry داخلي
            ...
            if "429" in str(e):
                wait = 30  # 🔥 انتظار طويل جداً في حالة 429
            else:
                wait = 10
